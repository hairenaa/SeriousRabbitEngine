...
Model::Model(std::string path, Shader* _shader, Material* _material)
	:shader(_shader),
	material(_material)
{
	loadModel(path);
...
Model::Model(std::string path, Shader* _shader)
	:shader(_shader)
{
...
Mesh Model::processMesh(aiMesh * aimesh, const aiScene * scene)
{
	...
	return Mesh(tempVertices, tempIndicies, tempTextures,material);

}



std::vector<Texture> Model::AiMaterialToTexture(aiMaterial *mat, aiTextureType type, TextureType textureType)
{	
...
unsigned int Model::LoadImageToGpu(const char* path)
{
